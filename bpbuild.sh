...
# configure tags ###############################################################################
build_image_w_tag=${BUILD_IMAGE_W_TAG:=ericsgagnon/buildpack-deps-cuda:ubuntu22.04-cuda11.7}
additional_image_tag=${ADDITIONAL_IMAGE_TAG:=ericsgagnon/buildpack-deps-cuda:ubuntu22.04-cuda11}
...
docker build --pull --no-cache=true -t ${build_image_w_tag}  -f ${build_context}/BPBasedDockerfile . 
build_exit_code=$?
